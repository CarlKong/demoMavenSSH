...
public class ErrorCode {
    ...
    public static String getLoggerInfo(String errorCodeId) {
        if (errorCodes == null) {
            errorCodes = ReaderXmlUtils.getErrorCodes();
            ErrorCode errorCode = errorCodes.get(errorCodeId);
            return "<ErrorLevel:>" + errorCode.getFlag() + "<ErrorMessage:>" + errorCode.getErrorMessageKey() + "<ErrorCode:>"
                    + errorCode.getErrorCodeId();
        } else {
            ErrorCode errorCode = errorCodes.get(errorCodeId);
            return "<ErrorLevel:>" + errorCode.getFlag() + "<ErrorMessage:>" + errorCode.getErrorMessageKey() + "<ErrorCode:>"
                    + errorCode.getErrorCodeId();
